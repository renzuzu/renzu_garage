...
function OpenGarage(garageid,garage_type,jobonly,default)
    ...
    if cars > 0 then
        ...
        if not propertyspawn.x and #(GetEntityCoords(cache.ped) - vector3(garagecoord[tid].garage_x,garagecoord[tid].garage_y,garagecoord[tid].garage_z)) > 15 then
        ...
        elseif propertyspawn.x and #(GetEntityCoords(cache.ped) - vector3(propertyspawn.x,propertyspawn.y,propertyspawn.z)) > 15 then
            SetEntityCoords(cache.ped, propertyspawn.x,propertyspawn.y,propertyspawn.z, false, false, false, true)
        end
        CloseNui()
    end
...
function SetVehicleProp(vehicle, mods)
    ...
    if Config.ReturnDamage and DoesEntityExist(vehicle) then
        ...
        if mods.vehicle_window ~= nil then
        ...
        if mods.vehicle_doors ~= nil then
            for doorid = 0, 5, 1 do
    ...
    if Config.use_RenzuCustoms then
        ...
        if DoesEntityExist(vehicle) then
            ...
            if props.sound ~= nil then ForceVehicleEngineAudio(vehicle, props.sound) end
	    ...
	    if props.customSecondaryColor ~= nil then SetVehicleCustomSecondaryColour(vehicle, props.customSecondaryColor[1], props.customSecondaryColor[2], props.customSecondaryColor[3]) end
            ...
            if props.fuelLevel ~= nil then SetVehicleFuelLevel(vehicle, props.fuelLevel + 0.0) if DecorGetFloat(vehicle,'_FUEL_LEVEL') then DecorSetFloat(vehicle,'_FUEL_LEVEL',props.fuelLevel + 0.0) end end
        end
    end
end
...
function GetVehicleProperties(vehicle)
    if Config.use_RenzuCustoms then
        ...
        mods.deformation = json.encode(GetVehicleDeformation(vehicle))
        return mods
    else
        if DoesEntityExist(vehicle) then
            -- https://github.com/esx-framework/es_extended/tree/v1-final COPYRIGHT
...
-- DEFORMATION VEHICLE
--https://github.com/Kiminaze/VehicleDeformation/blob/main/deformation.lua awsome
--[[
	Custom license
